    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-border bg-card p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="space-y-1"><Label>{t("rec.day")}</Label><Input type="number" min={1} max={31} value={day} onChange={(e) => setDay(Number(e.target.value))} /></div>
        </div>
      ...
      <ul className="divide-y divide-border rounded-2xl border border-border bg-card">
          ...
          return (
            <li key={r.id} className="flex items-center justify-between p-3 text-sm">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{r.day_of_month} · {def.label} · {formatMoney(Number(r.amount))}</p>
              </div>
        ...
        {(!list.data || list.data.length === 0) && <li className="p-3 text-sm text-muted-foreground">{t("rec.none")}</li>}
      </ul>
    </div>
  );
}